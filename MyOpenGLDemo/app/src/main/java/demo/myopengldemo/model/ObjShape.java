...
public class ObjShape {
    ...
    private String vertexShaderCode = "uniform mat4 uMVPMatrix;"
            ...
            + "vec3 newNormal = normalize((uMMatrix*vec4(normalTarget,1)).xyz) - normalize((uMMatrix*vec4(aPosition,1)).xyz);"
            + "vec3 vectorLight = normalize(uLightLocation - (uMMatrix * vec4(aPosition,1)).xyz);"
            + "vectorLight = normalize(vectorLight);"
            + "float factor = max(0.0, dot(newNormal, vectorLight));"
