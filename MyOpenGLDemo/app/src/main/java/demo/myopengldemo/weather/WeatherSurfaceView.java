...
public class WeatherSurfaceView extends GLSurfaceView {
    ...
    float ty=0;//目标点y位置
    float tz=0;//目标点z位置
    public float currSightDis=40;//摄像机和目标的距离
    ...
    public float lightAzimuth=90;//灯光的方位角

    CollisionShape boxShape;//共用的立方体
    CollisionShape planeShape;//共用的平面形状
    DiscreteDynamicsWorld dynamicsWorld;//世界对象
    WeatherModel mWeatherModel;//骰子

    ArrayList<WeatherModel> mWeatherModelList = new ArrayList<>();
...
//                    for(WeatherModel di:mWeatherModelList){
//                        synchronized (di) {
...
//            MatrixState.rotate(-90,1,0,0);
            ...
            synchronized (mWeatherModelList) {
                for (WeatherModel di : mWeatherModelList) {
                    synchronized (di) {
                        //绘制骰子 中间
                        MatrixState.pushMatrix();
...
//                mWeatherModel.body.forceActivationState(RigidBody.WANTS_DEACTIVATION);
                mWeatherModelList.add(mWeatherModel);
            }
