...
public class WeatherModel {
    ...
    public WeatherModel(Context context, float[] verticesData, float[] verticesNormalData, float[] textureCoordinateData){
        mContext = context;
    ...
    private void initShader(){
        mProgram = demo.myopengldemo.weather.MyGLUtils.buildProgram(mContext, R.raw.weather_scene_vertex, R.raw.weather_scene_frag);

        mTextureId = demo.myopengldemo.weather.MyGLUtils.loadTexture(mContext,R.drawable.touzi,new int[2]);
    ...
    public void drawSelf(int isShadow) {
        ...
        if(isShadow == 0) {
            ...
            //进行移位变换
            demo.myopengldemo.weather.MatrixState.translate(trans.origin.x, trans.origin.y, trans.origin.z);
            Quat4f ro = trans.getRotation(new Quat4f());//获取当前变换的旋转信息
            if (ro.x != 0 || ro.y != 0 || ro.z != 0) {
                float[] fa = demo.myopengldemo.weather.MyGLUtils.fromSYStoAXYZ(ro);//将四元数转换成AXYZ的形式
                demo.myopengldemo.weather.MatrixState.rotate(fa[0], fa[1], fa[2], fa[3]);//执行旋转
            }
        }
        ...
        //将最终变换矩阵传入着色器程序
        GLES20.glUniformMatrix4fv(mMVPMatrixHandle, 1, false, demo.myopengldemo.weather.MatrixState.getFinalMatrix(), 0);
        //将位置、旋转变换矩阵传入着色器程序
        GLES20.glUniformMatrix4fv(mModelMatrixHandle, 1, false, demo.myopengldemo.weather.MatrixState.getMMatrix(), 0);
        //将光源位置传入着色器程序
        GLES20.glUniform3fv(mLightPositionHandle, 1, demo.myopengldemo.weather.MatrixState.lightPositionFB);
        //将摄像机位置传入着色器程序
        GLES20.glUniform3fv(mCameraPositionHandle, 1, demo.myopengldemo.weather.MatrixState.cameraFB);
        //将是否绘制阴影属性传入着色器程序
        GLES20.glUniform1i(mIsShadowHandle, isShadow);
        //将投影、摄像机组合矩阵传入着色器程序
        GLES20.glUniformMatrix4fv(mVPMatrixHandle, 1, false, demo.myopengldemo.weather.MatrixState.getViewProjMatrix(), 0);
